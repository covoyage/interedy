...
export const Sidebar: React.FC<Props> = ({ connections, activeId, currentDB, onSelect, onSwitchDB, onRefresh, collapsed, onToggleCollapse }) => {
  ...
  const handleConnect = async (id: string) => {
    ...
    try {
      ...
      setConnectErrors((prev) => ({ ...prev, [id]: e?.toString() || t.sidebar.connectFail }));
    }
    setConnecting(null);
  };
        ...
        {connections.map((conn) => (
          <div key={conn.id}>
            ...
            {connectErrors[conn.id] && (
              <div className="mx-3 mb-1 px-2 py-1.5 text-xs text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded break-all">
                {connectErrors[conn.id]}
              </div>
            )}
          </div>
